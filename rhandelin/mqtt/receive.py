...
class Receiver():
    ...
    def __init__(self):
        ...
        # more callbacks, etc
        self.client = mqtt.Client(client_id = str(self.id))
        ...
        self.commands_dict = {
        #'my_coords':
        'rainbow':self.rainbow_rand,
        'increment':self.increment_simulation
        }
    ...
    def rainbow_rand(self):
        for iter_id in range(200):
            pixels.fill(((self.id + iter_id % 2)*255, (self.id + iter_id + 2 % 7)*42,
            (self.id + 5 + iter_id % 9)*28))
            pixels.show()
            time.sleep(0.5)
...
#Start Receiver
myReceiver = Receiver()
myReceiver.client.loop_forever()